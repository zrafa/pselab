...
#define RX_INTERRUPT_ENABLE 0x80
#define	UDRE_INTERRUPT_ENABLE 0x20


int channelTx,channelRx;
                 ...
                 CircBuf * pTxQueue, CircBuf * pRxQueue)
{
   ...
   //
    ...
    sei(); //habilita las interrumpciones globales, es decir el bit 7 del registro SREG.
...
void SCC::txStart(int channel)
 {
    channelTx=channel; //como no se le puede pasar parametros del channel utilizado a la rutina que controla la interrupcion utilizo un channelTx global.

    puerto_serial -> status_control_b = (uint8_t) (puerto_serial -> status_control_b)| UDRE_INTERRUPT_ENABLE; //se habilita la interrupcion Data Registry Empty.
...
void SCC::rxStart(int channel)
 {
   channelRx=channel;
    puerto_serial->status_control_b = (puerto_serial->status_control_b) | RX_INTERRUPT_ENABLE;

 }    /* rxStart() */


 ISR(USART_RX_vect){
    rxQueue[channelRx]->add (puerto_serial->data_es);
    puerto_serial->status_control_b = (puerto_serial->status_control_b) & ~(RX_INTERRUPT_ENABLE);// desactivo la interrupcion de RX.

 }


 ISR(USART_UDRE_vect){
 if(txQueue[channelTx]->isEmpty() == 0){
               puerto_serial -> data_es =txQueue[channelTx]->remove();
}
   }
