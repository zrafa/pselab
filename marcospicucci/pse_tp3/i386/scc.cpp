...
SCC::txStart(int channel)
{
	...
	while(txQueue[channel]->isEmpty() != 1){
		printf("%c",txQueue[channel]->remove());//ojo el print
	}	
}   /* txStart() */
...
SCC::rxStart(int channel)
{
	...
	//leo con getc y guardo en el buffer
	rxQueue[channel]-> add((item)getc(stdin));
	//printf("obtengo y guardo, rxtart de scc");
	}
